...
'activeFont              byte Header#_5_x_7_FONT 'SIMPLYTROINICS_FONT 'FREE_DESIGN_FONT '
...
PUB Start
  ...
  memoryOffset := @datOffset - datOffset ' Add memoryOffset to correct pointer
  ...
  {D
  Pst.str(string(11, 13, "OLED Driver Starting"))
  C  }
...
PUB ScrollString(localStr, pstFlag)
  ...
  if activeScrollRow < Header#OLED_LINES - 1
    ...
    L
    Pst.str(string(11, 13, "Calling ScrollBuffer"))
    C
    ScrollBuffer(1)

  L
  Pst.str(string(11, 13, "activeScrollRow = "))
  Pst.Dec(activeScrollRow)
  C  
  Write4x16String(localStr, strsize(localStr) <# 16, 0, activeScrollRow)
  if pstFlag
    L
    Pst.ClearEnd
    Pst.Newline
...
PUB WriteOledString(str, len, col, row, transparentFlag) {| characterSize, {bufferAddress, 
 eightByteBuffer[2], tempPtr,} localDebugFlag   }
  ...
  'tempPtr := Spi.GetBuffer +  (row * 16) + col
  ...
  repeat len
    ...
    WriteChar(byte[str], col, row, transparentFlag)
    ''characterFlag := 1
    {
...
'if fontHeight == 8
 '   Write4x16String(str, len, col, row)
PUB WriteChar(character, col, row, transparentFlag) '| eightByteBuffer[2], bufferAddress
'' For now all the characters have to be 8 pixels by 8 pixels.
 ...
 { if oledFileType <> Header#FONT_OLED_TYPE
  ...
  Spi.Write5x7Char(character, col, row) 
  D
...
PUB Write4x16String(str, len, col, row)

  'Write4x16String(str, len, col, row)
  'if activeFont <> Header#_5_x_7_FONT
    'SetFont(Header#_5_x_7_FONT)
  WriteOledString(str, len, col, row, 0)

'PUB Write4x16String(str, len, col, row)

  'Spi.Write4x16String(str, len, col, row)
...
PRI OledDemo(frozenState) | h, i, j, k, q, r, s, count
  ...
  repeat
    ...
    Spi.clearDisplay
                               '0123456789012345
    Spi.write4x16String(String("Based on code"), strsize(String("Based on code")), 0, 2)
    Spi.write4x16String(String("from . . ."), strsize(String("from . . .")), 2, 4)
    UpdateDisplay
    ...
    repeat 1024
      if Spi.GetDisplayType == Spi#TYPE_128X32
        'Spi.Write5x7Char
        WriteChar(GetRandomChar, ||q? // 16, ||k? // 4, 0)
        
      else
        'Spi.Write5x7Char
        WriteChar(GetRandomChar, ||q? // 16, ||k? // 8, 0)
      UpdateDisplay
...
PRI PropLogo(frozenState)
  ...
  BounceBitmapNumber(frozenState, BEANIE_SMALL_BITMAP, {
  } 100, 40, -1, -1, 0, 0, 0, 0, 1)
  ...
  repeat
    ...
    repeat row from 0 to maxIndex
      if true 'long[long[dataPtrPtr][row]] <> previousValue[row]
        ...
        'Spi.W
        Write4x16String(@localBuffer, 16, 0, row)
...
PUB FitBitmapId(bitmapIndex, offsetX, offsetY, transparentFlag)
  ...
  result := @propBeanie 'bitmapPointer[bitmapIndex] + memoryOffset
  FitBitmap(Spi.GetBuffer, Header#OLED_WIDTH, Header#OLED_HEIGHT, {
  } result, bitmapWidth[bitmapIndex], {
  } bitmapHeight[bitmapIndex], offsetX, offsetY, transparentFlag)
  ...
  repeat sourceRows 
    ...
    if byteOffset 'true '
      ...
      if activeDestPtr => outOfBounds[0] and activeDestPtr + sourceWidth < outOfBounds[1] ' full line in bounds
        MoveOrOrBytes(activeDestPtr, tempSourcePtr, sourceWidth, transparentFlag)
      elseif activeDestPtr < outOfBounds[0] and activeDestPtr + sourceWidth => outOfBounds[0] ' end of line in bounds
        ...
        MoveOrOrBytes(outOfBounds[0], tempSourcePtr + result, sourceWidth - result, transparentFlag)
      elseif activeDestPtr < outOfBounds[1] ' beginning of line in bounds
        result := outOfBounds[1] - activeDestPtr
        MoveOrOrBytes(activeDestPtr, tempSourcePtr, result, transparentFlag)
...
'' uninverted.
  ...
  if invertFlag
    L
    Pst.str(string(11, 13, "UpdateDisplay, invertFlag = "))
    Pst.Dec(invertFlag)
    C
    InvertBlock
  else
...
datOffset     word @datOffset

bitmapPointer word @propBeanie
...
{propBeanie    byte $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF, $FF
